...
import { useStyles } from './CompletedList.styles';
...
export const CompletedList = () => {
  ...
  const fetchData = useCallback(async (offset: number) => {
    try {
      setIsLoading(true);
      const { orders } = await networkService.getAllOrders(offset, ORDER_TAB_STATUS.COMPLETED);
      if (orders.length) {
  ...
  const renderItem = useCallback(({ item }: { item: Order}) => (
    <ManagerOrderCard
      order={item}
      buttonTitle={t('CargoList_completed_button')}
      t={t}
      onPress={onEditPress}
    />
  ), [onEditPress, t]);
